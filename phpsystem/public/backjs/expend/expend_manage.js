...
$(function () { 
	...
	$("#expend_manage").datagrid({
		url : backURL + getVisitPath("Expend") + '/backList',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
			...
			handler : function () {
				if ($("#expendEditForm").form("validate")) {
					...
					if(!$("#expendEditForm").form("validate")) {
						...
						$("#expendEditForm").form({
						    url: backURL + getVisitPath("Expend") + "/update",
						    onSubmit: function(){
								if($("#expendEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initExpendManageTool() {
	expend_manage_tool = {
		init: function() {
			$.ajax({
				url : backURL + getVisitPath("ExpendType") + "/listAll",
				type : "post",
				dataType: "json",
			...
			$.ajax({
				url : backURL + getVisitPath("PayWay") + "/listAll",
				type : "post",
				dataType: "json",
				success : function (data, response, status) {
		...
		exportExcel: function() {
			$("#expendQueryForm").form({
			    url: backURL + getVisitPath("Expend") + "/outToExcel",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url :  backURL + getVisitPath("Expend") + "/deletes",
							data : {
						...
						if (expend) { 
							...
							$("#expend_exprendTypeObj_expendTypeId_edit").combobox({
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#expend_exprendTypeObj_expendTypeId_edit").combobox("select", expend.exprendTypeObj);
							...
							$("#expend_payWayObj_payWayId_edit").combobox({
						        ...
						        editable: false, //不允许手动输入 
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#expend_payWayObj_payWayId_edit").combobox("select", expend.payWayObj);
							...
							$("#expend_userObj_user_name_edit").combobox({
						        ...
						        editable: false, //不允许手动输入 
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#expend_userObj_user_name_edit").combobox("select", expend.userObj);
