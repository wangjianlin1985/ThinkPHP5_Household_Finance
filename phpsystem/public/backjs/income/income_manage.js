...
$(function () { 
	...
	$("#income_manage").datagrid({
		url : backURL + getVisitPath("Income") + '/backList',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
			...
			handler : function () {
				if ($("#incomeEditForm").form("validate")) {
					...
					if(!$("#incomeEditForm").form("validate")) {
						...
						$("#incomeEditForm").form({
						    url: backURL + getVisitPath("Income") + "/update",
						    onSubmit: function(){
								if($("#incomeEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initIncomeManageTool() {
	income_manage_tool = {
		init: function() {
			$.ajax({
				url : backURL + getVisitPath("IncomeType") + "/listAll",
				type : "post",
				dataType: "json",
		...
		exportExcel: function() {
			$("#incomeQueryForm").form({
			    url: backURL + getVisitPath("Income") + "/outToExcel",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url :  backURL + getVisitPath("Income") + "/deletes",
							data : {
						...
						if (income) { 
							...
							$("#income_incomeTypeObj_typeId_edit").combobox({
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#income_incomeTypeObj_typeId_edit").combobox("select", income.incomeTypeObj);
							...
							$("#income_payWayObj_payWayId_edit").combobox({
						        ...
						        editable: false, //不允许手动输入 
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#income_payWayObj_payWayId_edit").combobox("select", income.payWayObj);
							...
							$("#income_userObj_user_name_edit").combobox({
						        ...
						        editable: false, //不允许手动输入 
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#income_userObj_user_name_edit").combobox("select", income.userObj);
